<?php
class KwcIntegratorTemplate_Kwc_Advanced_IntegratorTemplate_ContentSender extends Kwf_Component_Abstract_ContentSender_Default
{
    protected function _render($includeMaster, &$hasDynamicParts)
    {
        if (!isset($_GET['embed'])) {
            header('Location: '.$this->_data->getComponent()->getRow()->url);
            exit;
        } else {
